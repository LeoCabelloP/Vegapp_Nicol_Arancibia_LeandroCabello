...
bool dvda_titleset_t::open(dvda_zone_t* dvda_zone, int i_titleset) {
	...
	char file_name[13];
	sprintf_s(file_name, sizeof(file_name), "ATS_%02d_0.IFO", titleset);
	...
	if (atsi_size >= 0x0800) {
		if (atsi_file->read((char*)&atsi_mat, sizeof(atsi_mat_t)) == sizeof(atsi_mat_t)) {
			if (memcmp("DVDAUDIO-ATS", atsi_mat.ats_identifier, 12) == 0) {
				...
				for (int i = 0; i < 9; i++) {
					sprintf_s(file_name, sizeof(file_name), "ATS_%02d_%01d.AOB", titleset, i + 1);
...
DVDAERROR dvda_titleset_t::get_block(uint32_t block, uint8_t* buf_ptr) {
	for (int i = 0; i < 9; i++) {
		if (aobs[i].dvda_fileobject && block >= aobs[i].block_first && block <= aobs[i].block_last) {
			...
			if (dvda_zone->get_cpxm_context()->media_type > 0)
				dvdcpxm_decrypt(dvda_zone->get_cpxm_context(), buf_ptr, 1, DVDCPXM_PRESERVE_CCI);
			return DVDAERR_OK;
		}
	}
...
int dvda_titleset_t::get_blocks(uint32_t block_first, uint32_t block_last, uint8_t* buf_ptr) {
	...
	if (dvda_zone->get_cpxm_context()->media_type > 0)
		dvdcpxm_decrypt(dvda_zone->get_cpxm_context(), buf_ptr, blocks_read, DVDCPXM_PRESERVE_CCI);
	return blocks_read;
}
...
bool dvda_zone_t::open(dvda_filesystem_t* dvda_filesystem) {
	EnterCriticalSection(&lock);
	if (is_open)
		close();
	obj_type = DVDATypeZone;
	this->dvda_filesystem = dvda_filesystem;
	amgi_mat_t amgi_mat;
	audio_titlesets = 99;
	video_titlesets = 99;
	const char* drive = strstr(dvda_filesystem->get_path(), "://");
	if (drive)
		dvdcpxm_init(&cpxmContext, (char*)(drive + 3));
