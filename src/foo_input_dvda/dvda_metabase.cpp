...
auto subsong_to_string = [](auto subsong) {
	return string_printf("%d.%d.%d", (subsong >> 16) & 0xff, (subsong >> 8) & 0xff, subsong & 0xff);
};

string8 get_md5(dvda_filesystem_t* p_filesystem) {
	auto md5_string{ string8() };
	auto tag_file = p_filesystem->file_open("AUDIO_TS.IFO");
	if (tag_file) {
		auto tag_size = (int)tag_file->get_size();
		vector<uint8_t> tag_data;
		tag_data.resize(tag_size);
		if (tag_file->read(tag_data.data(), tag_size) == tag_size) {
			md5_string = hasher_md5::get()->process_single(tag_data.data(), tag_size).asString();
		}
		else {
		...
		console::error("Cannot open MD5 hash source");
	}
	return md5_string;
}

dvda_metabase_t::dvda_metabase_t(dvda_filesystem_t* p_filesystem, const char* p_metafile) {
	initialized = false;
	store_id = get_md5(p_filesystem);
	...
	if (!p_metafile) {
		if (!filesystem::g_exists(store_path, media_abort)) {
			...
			if (filesystem::g_exists(store_path, media_abort)) {
				console::info(string_printf("DVDA metabase folder '%s' has been created", store_path.c_str()));
			}
			else {
				popup_message::g_show(string_printf("Cannot create DVDA metabase folder '%s'", store_path.c_str()), METABASE_CATALOG, popup_message::icon_error);
			}
		}
	}
	else {
		if (!filesystem::g_exists(p_metafile, media_abort)) {
			if (filesystem::g_exists(store_file, media_abort)) {
				...
				if (filesystem::g_exists(p_metafile, media_abort)) {
					console::info(string_printf("DVDA metabase file is copied from '%s' to '%s'", store_file.c_str(), p_metafile));
				}
				else {
					popup_message::g_show(string_printf("Cannot copy DVDA metabase file to '%s'", p_metafile), METABASE_CATALOG, popup_message::icon_error);
				}
			}
		}
	}
...
void dvda_metabase_t::get_track_info(t_uint32 track_number, file_info& track_info) {
	string_formatter(node_id);
	node_id = subsong_to_string(track_number);
...
void dvda_metabase_t::set_track_info(t_uint32 track_number, const file_info& track_info) {
	if (track_number & 0x01000000) {
		return;
	}
	string_formatter(node_id);
	node_id = subsong_to_string(track_number);
	...
	delete_tags(node_track, TAG_META);
	...
	delete_tags(node_track, TAG_REPLAYGAIN);
	auto replaygain_info{ track_info.get_replaygain() };
...
void dvda_metabase_t::get_albumart(t_uint32 albumart_id, vector<t_uint8>& albumart_data) {
	string_formatter(node_id);
	node_id << albumart_id;
...
void dvda_metabase_t::set_albumart(t_uint32 albumart_id, const vector<t_uint8>& albumart_data) {
	string_formatter(node_id);
	node_id << albumart_id;
...
bool dvda_metabase_t::init_xmldoc(const char* store_type) {
	if (V_VT(&xmlfile) != VT_BSTR) {
		return false;
	}
	...
	if (hr != S_OK || is_ok != VARIANT_TRUE) {
		...
		auto comm_root{ xmldoc->createComment("DVD-Audio metabase file") };
		if (!comm_root) {
			return false;
		}
...
void dvda_metabase_t::delete_tags(IXML_NodePtr node, const char* tag_type) {
...
void dvda_metabase_t::insert_tag(IXML_NodePtr node, const char* tag_type, const char* tag_name, const char* tag_value) {
...
IXML_NodePtr dvda_metabase_t::get_node(const char* tag_type, const char* att_id, bool create) {
...
IXML_NodePtr dvda_metabase_t::new_node(const char* tag_type, const char* att_id) {
