...
bool sacd_media_disc_t::open(file_ptr filehint, const char* path, t_input_open_reason reason) {
	open_reason = reason;
	TCHAR device[8];
...
file_ptr sacd_media_disc_t::get_handle() {
	return file_ptr();
}
...
t_input_open_reason sacd_media_disc_t::get_reason() {
	return open_reason;
}
...
bool sacd_media_file_t::open(file_ptr filehint, const char* path, t_input_open_reason reason) {
	input_open_file_helper(filehint, path, reason, media_abort);
	media_file = filehint;
	open_reason = reason;
...
file_ptr sacd_media_file_t::get_handle() {
	return media_file;
}
...
t_input_open_reason sacd_media_file_t::get_reason() {
	return open_reason;
}
