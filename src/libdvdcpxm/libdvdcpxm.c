...
{
	{0x00, 0x2f90, 0x55114d103e080c},
	{0x00, 0x2814, 0x03ba211ae3108e},
...
int cppm_set_id_album(CPxMContext *p_ctx)
{
...
int cprm_set_id_media(CPxMContext *p_ctx)
{
	...
	if (ioctl_ReadCPRMMediaId(&css.agid, p_buffer))
		return -1;
	p_ctx->id_album_media = *(uint64_t*)&(((cprm_media_id_t*)p_buffer)->id_media);
	B2N_64(p_ctx->id_album_media);
	return 0;
}
...
int vr_get_k_te(CPxMContext *p_ctx, char *psz_vr_mangr)
{
	FILE    *f_vr_m;
	...
	if (fread(vmgi_mat, 1, 512, f_vr_m) == 512)
	{
		if (memcmp(vmgi_mat, "DVD_RTR_VMG0", 12) == 0)
		{
			...
			{
				p_ctx->vr_k_te = *(uint64_t*)&vmgi_mat[268]; /* Get Kte */
				B2N_64(p_ctx->vr_k_te);
			}
			ret = 0;
		}
	}
...
int dvdcpxm_init(CPxMContext *p_ctx, char *psz_target)
{
	char psz_dvddev[8];
	...
	p_ctx->media_type = copyright.CopyrightProtectionType;
	...
	case COPYRIGHT_PROTECTION_CPPM:
		if (cppm_set_id_album(p_ctx) == 0)
		{
			...
			if (p_mkb)
			{
				ret = process_mkb(p_mkb, cppm_device_keys, sizeof(cppm_device_keys) / sizeof(*cppm_device_keys), &p_ctx->media_key);
				free(p_mkb);
				if (ret) break;
			}
		}
	...
	case COPYRIGHT_PROTECTION_CPRM:
		if (cprm_set_id_media(p_ctx) == 0)
		{
			p_mkb = cprm_get_mkb();
			if (p_mkb)
			{
				ret = process_mkb(p_mkb, cprm_device_keys, sizeof(cprm_device_keys) / sizeof(*cprm_device_keys), &p_ctx->media_key);
				free(p_mkb);
				if (ret) break;
			}
			strcpy(psz_file, "?:\\DVD_RTAV\\VR_MANGR.IFO");
			psz_file[0] = psz_target[0];
			ret = vr_get_k_te(p_ctx, psz_file);
			if (ret)
			{
...
int cppm_decrypt_block(CPxMContext *p_ctx, uint8_t *p_buffer, int flags)
{
	...
	{
		k_au = c2_g(p_ctx->id_album_media, p_ctx->media_key) & 0x00ffffffffffffff;
...
int cprm_decrypt_block(CPxMContext *p_ctx, uint8_t *p_buffer, int flags)
{
	uint64_t d_kc_i, k_au, k_i, k_c;
	int encrypted;
...
int dvdcpxm_decrypt(CPxMContext *p_ctx, void *p_buffer, int nr_blocks, int flags)
{
	int i;
	int encrypted = 0;
	...
	case COPYRIGHT_PROTECTION_CPPM:
		for (i = 0; i < nr_blocks; i++)
			encrypted += cppm_decrypt_block(p_ctx, (uint8_t*)p_buffer + i * DVDCPXM_BLOCK_SIZE, flags);
		return encrypted;
	case COPYRIGHT_PROTECTION_CPRM:
		for (i = 0; i < nr_blocks; i++)
			encrypted += cprm_decrypt_block(p_ctx, (uint8_t*)p_buffer + i * DVDCPXM_BLOCK_SIZE, flags);
		return encrypted;
	}
	return 0;
}
